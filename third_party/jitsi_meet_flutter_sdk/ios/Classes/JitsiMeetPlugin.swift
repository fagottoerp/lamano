...
public class JitsiMeetPlugin: NSObject, FlutterPlugin, FlutterStreamHandler {
    ...
    private func currentRootViewController() -> UIViewController? {
        ...
        return topViewController(from: sceneRoot ?? appDelegateRoot)
    }
    ...
    private func join(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        guard let presenter = currentRootViewController() else {
            result(FlutterError(code: "NO_ROOT_VIEW_CONTROLLER", message: "Unable to resolve a root view controller for Jitsi presentation", details: nil))
            return
        }

        let sink = eventSink ?? { _ in }
